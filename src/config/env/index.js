...
module.exports = (api, options) => {
  ...
  if (options.verbose) {
    if (
      process.env[options.envName] === 'production' ||
      process.env[options.envName] === 'development'
    ) {
      console.error(
        'APP_ENV error',
        'cannot use APP_ENV=development or APP_ENV=production',
      );
    }
  }
  ...
  env = options.safe
    ? safeObjectAssign(
        undefObjectAssign(
          undefObjectAssign(undefObjectAssign(parsed, modeParsed), localParsed),
          modeLocalParsed,
        ),
        dotenvTemporary,
        ['NODE_ENV', 'BABEL_ENV', options.envName],
      )
    : undefObjectAssign(
        undefObjectAssign(
          undefObjectAssign(undefObjectAssign(parsed, modeParsed), localParsed),
        ...
        if (path.node.source.value === options.moduleName) {
          for (const [index, specifier] of path.node.specifiers.entries()) {
            if (specifier.type === 'ImportDefaultSpecifier') {
              throw path
                .get('specifiers')
                [index].buildCodeFrameError('Default import is not supported');
            }

            if (specifier.type === 'ImportNamespaceSpecifier') {
              throw path
                .get('specifiers')
                [index].buildCodeFrameError('Wildcard import is not supported');
            }
            ...
            if (specifier.imported && specifier.local) {
              ...
              if (
                ...
                !options.allowlist.includes(importedId)
              ) {
                throw path
                  .get('specifiers')
                  [index].buildCodeFrameError(
                    `"${importedId}" was not present in allowlist`,
                  );
              } else if (
                Array.isArray(options.whitelist) &&
                !options.whitelist.includes(importedId)
              ) {
                console.warn(
                  '[DEPRECATION WARNING] This option is will be deprecated soon. Use allowlist instead',
                );
                throw path
                  .get('specifiers')
                  [index].buildCodeFrameError(
              ...
              if (
                Array.isArray(options.blocklist) &&
                options.blocklist.includes(importedId)
              ) {
                throw path
                  .get('specifiers')
                  [index].buildCodeFrameError(
                    `"${importedId}" was not present in blocklist`,
                  );
              } else if (
                Array.isArray(options.blacklist) &&
                options.blacklist.includes(importedId)
              ) {
                console.warn(
                  '[DEPRECATION WARNING] This option is will be deprecated soon. Use blocklist instead',
                );
                throw path
                  .get('specifiers')
                  [index].buildCodeFrameError(
                    `"${importedId}" was blacklisted`,
                  );
              }
              ...
              if (!options.allowUndefined && !Object.hasOwn(env, importedId)) {
                throw path
                  .get('specifiers')
                  [index].buildCodeFrameError(
                    `"${importedId}" is not defined in ${options.path}`,
                  );
              }
      ...
      MemberExpression(path) {
        if (path.get('object').matchesPattern('process.env')) {
          ...
          if (t.isStringLiteral(key)) {
            ...
            const value =
              env && importedId in env
                ? env[importedId]
                : process.env[importedId];
            if (value !== undefined) {
